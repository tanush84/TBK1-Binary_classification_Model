...
def main():
    ...
    if df is not None:
        ...
        X1 = df.drop('SMILES', axis=1)
        X2 = X1.drop('ROMol', axis=1)
        X3 = X2.drop('FP', axis=1)
        #print(X3.head(5))
        
        filename = './asset/my_model_mlp-rdkit.joblib'
        # load the model from disk
        loaded_model = joblib.load(filename)

        y_train=pd.read_csv('./asset/tbk1_Y_train-rdkit.csv')
        X_train=pd.read_csv('./asset/tbk1_X_train-rdkit.csv')
        result = loaded_model.fit(X_train, y_train)
        y_predicted = loaded_model.predict(X3)
